...
Animal::Animal(): Animal("Noname animal", "No specie", 0, true, 0)
{}

Animal::Animal(std::string name, std::string species, int age, bool isMale, WORD color): name(name), species(species), age(age), isMale(isMale), color(color)
{
...
void Animal::SetXCoordinate(int x)
{
	this->dx = x;
}

void Animal::SetYCoordinate(int y)
{
	this->dy = y;
}

void Animal::SetActionText(std::string text)
{
...
int Animal::GetXCoordinate() const
{
	return x;
}

int Animal::GetYCoordinate() const
{
	return y;
}
