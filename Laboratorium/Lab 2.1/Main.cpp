...
{
	// create dogs
	Dog fafik("Fafik", 6, "Alabai", 10, 10, true); 
	...
	std::cout << "Hugo age before operators is " << hugo.GetAge() << std::endl;
	// operatory ++
	...
	// sprawdŸmy !!!
		...
		std::cout << "Hugo age after hugo++ is " << hugo.GetAge() << std::endl;
		try {
			Dog bobik = hugo + fafik;
			bobik.SetName("Bobik");
		}
		catch (string sameSex)
		{
			std::cout << sameSex << std::endl;
		}
